...
void init_random(unsigned int the_seed){
...
//	delete[] gotya;gotya=NULL;
}

void randomize(int x[],unsigned long N){
    int *x_old;
    //short int *gotya;
    unsigned long i,j,n;
...
//Returns anormally distributed deviate with zero mean and unit variance, 
 ...
 if (iset == 0) { //We don’t have an extra deviate handy, so 
	do { v1=2.0*ran2(&idum_ACT)-1.0; //pick two uniform numbers in the square extending from -1 to +1 in each direction,
...
float gammln(float xx){
	...
	// Internal arithmetic will be done in double precision, anicety that you 
